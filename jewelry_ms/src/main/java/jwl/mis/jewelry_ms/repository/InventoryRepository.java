...
public interface InventoryRepository extends JpaRepository<Inventory,String> {
    ...
    @Query("SELECT i.sellingPrice FROM Inventory i WHERE i.itemId = :itemId")
    Double findSellingPriceByItemId(@Param("itemId") Long itemId);

    @Modifying
    @Transactional
    @Query("UPDATE Inventory i SET i.availableStock = i.availableStock - :qty WHERE i.itemId = :itemId")
